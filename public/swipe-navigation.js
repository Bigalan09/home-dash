...
class SwipeNavigationManager {
    ...
    handleTouchMove(event) {
        // Only process if we have exactly 3 touches
        if (event.touches.length !== this.requiredFingers || this.touches.size !== this.requiredFingers) {
            return;
        }
    ...
    handleTouchEnd(event) {
        ...
        const avgDeltaX = touchArray.reduce((sum, t) => sum + (t.currentX - t.startX), 0) / touchArray.length;
        const avgDeltaY = touchArray.reduce((sum, t) => sum + Math.abs(t.currentY - t.startY), 0) / touchArray.length;

        // Check if it's primarily a horizontal swipe with sufficient distance
        if (Math.abs(avgDeltaX) > this.swipeThreshold && Math.abs(avgDeltaX) > avgDeltaY * 2) {
    ...
    handleSwipe(direction) {
        ...
        // Navigation logic:
        // Swipe left = go forward (calendar → tasks)
        // Swipe right = go back (tasks → calendar)
        if (direction === 'left') {
            // Forward navigation
